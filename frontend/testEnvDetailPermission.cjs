...
// 测试函数
async function testEnvironmentDetailPermission() {
  const testResult = {
    success: false,
    details: {
      testSteps: []
  ...
  try {
    ...
    console.log(`  - 用户ID: ${loginResponse.data.id || loginResponse.data.user?.id}`);
    ...
    // 获取当前登录用户的ID
    const userId = loginResponse.data.id || loginResponse.data.user?.id || 'test';
    
    // 查找用户拥有的第一个环境（通过owner字段匹配）
    const userEnvironments = envList.filter(env => env.owner === userId.toString());
    let testEnvId = null;
    
    if (userEnvironments.length === 0) {
      console.log(`⚠️ 没有找到用户(ID: ${userId})拥有的环境，将尝试创建一个新环境...`);
      // 3. 创建测试环境
      const createStep = {
      ...
      testEnvId = createResponse.data.id;
    } else {
      console.log(`✅ 找到 ${userEnvironments.length} 个用户(ID: ${userId})拥有的环境`);
    ...
    console.log(`\n3. 尝试访问环境详情页...`);
    const detailStep = {
      step: 'access_environment_detail',
      status: 'running',
      environmentId: testEnvId
    };
    testResult.details.testSteps.push(detailStep);

    const detailResponse = await api.get(`/environments/${testEnvId}/`);
    ...
    if (detailResponse.status === 200) {
      console.log('✅ 环境详情页访问成功！权限验证通过！');
      console.log(`  - 环境名称: ${detailResponse.data.name}`);
      console.log(`  - 环境负责人: ${detailResponse.data.owner}`);
      console.log(`  - 当前登录用户ID: ${userId}`);
      testResult.success = true;
    } else {
      throw new Error(`环境详情页访问失败，状态码: ${detailResponse.status}`);
    }
...
// 执行测试
console.log('开始测试环境详情页访问权限...');
console.log('='.repeat(50));
testEnvironmentDetailPermission().then(result => {
  console.log('='.repeat(50));
  console.log('测试完成');
